...
% 5) If using mac, output files are currently saves as '.csv' and may need
% to be changed to '.xls' before the matching script
% 'Leaf_Physiology_Master_Sheet_Matching_V2_Main.m' that matches the
% outputs from this script with the master datasheet (which has leaf age)
...
for i=4:n1 % using the for-loop to automatically search each file.  Start at first curve file
    ...
    if str_name(1,1)=='A' % Only applicable for those ACI-curves
        ...
        t=mean(num(num(:,20)>0,20));    % leaf temperature is column 20 (this allows calculation of mean leaf temperature excluding missing values)
